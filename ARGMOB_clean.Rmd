...
author: 'Author: Tue Kjærgaard Nielsen'
...
refseq_tax_freq = as.data.frame(refseq_tax %>%
  count(Phylum,Class,Order,Family,Genus) %>%
  mutate(freq = (n / sum(n))*100))
...
p_dist_dens = ggplot(df_filt, aes(Ratio, Mean_distance, color = Replicon)) +
  stat_density_2d(aes(fill = stat(nlevel)), geom = "polygon", n = 200) +
  facet_wrap(. ~ Mechanism) +
  scale_fill_viridis_c("Scaled\ndensity",option = "E",alpha = 0.6) +
  theme_light() +
  ...
  ggtitle("A: Density plots of IS ratio against ARG-IS distance")
...
#Remmove ca. 400 low-abundance proteobacteria families out of 1605 total for testing
...
pheat = Heatmap(for_heatmap, top_annotation = ha, cluster_rows = F, name = "Ratio",heatmap_height = 0.1, column_dend_height = unit(30, "mm"),
...
p_med_high_aros = ggplot(high_subs_df, aes(x = reorder(Submechanism, -ARGMOB, FUN = median,), ARGMOB)) + 
  ...
  #ggtitle("Submechanisms with High and Very high ARG-MOB AROs") +
  ggtitle("c") +
